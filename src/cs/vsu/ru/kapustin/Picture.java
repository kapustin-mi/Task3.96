...
public class Picture {
    ...
    public SimpleColor getColor(double x, double y) {
        if (C1.isPointInsideCircle(x, y) && L1.isPointAboveLine(x, y)) {
            return getColorFromLargerSectionOfCircle(x, y);
        }
        if (!P2.isPointInsideParabola(x, y) && P3.isPointInsideParabola(x, y)) {
            return getColorFromThirdParabolaWithoutSecondParabola(x, y);
        }
        return getColorFromRemainderOfGraph(x, y);
    }

    private SimpleColor getColorFromLargerSectionOfCircle(double x, double y) {
        ...
        if (P1.isPointInsideParabola(x, y)) {
            return SimpleColor.GRAY;
        }
        return SimpleColor.ORANGE;
    }

    private SimpleColor getColorFromThirdParabolaWithoutSecondParabola(double x, double y) {
        if (C1.isPointInsideCircle(x, y) && !L1.isPointAboveLine(x, y)) {
            return SimpleColor.GRAY;
        }
        if (P1.isPointInsideParabola(x, y)) {
